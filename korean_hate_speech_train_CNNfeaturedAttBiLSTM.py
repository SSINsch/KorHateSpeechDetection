...
from models import CNNfeaturedAttBiLSTM
from utils import to_np, to_var, build_token_vocab, build_char_vocab, debug_result, plot_att_val
...
if __name__ == "__main__":
    ...
    # model
    cnn_feat_att_bilstm_model = CNNfeaturedAttBiLSTM(hidden_dim=HIDDEN_DIM,
                                                     num_lstm_layer=NUM_LSTM_LAYER,
                                                     n_classes=n_classes,
                                                     vocab_size=len(tok_vocab[1]),
                                                     char_vocab_size=len(char_vocab[1]))

    cnn_feat_att_bilstm_model = cnn_feat_att_bilstm_model.to(DEVICE)
    optimizer = torch.optim.Adam(cnn_feat_att_bilstm_model.parameters(), lr=learning_rate)
    ...
    for epoch in range(num_epoch):
        cnn_feat_att_bilstm_model.train()
        for step, (x_comment_tokenized, padded_word_tokens_matrix,
            ...
            # 모델에 입력
            pred, _ = cnn_feat_att_bilstm_model(padded_word_tokens_matrix, padded_char_tokens_matrix)
        ...
        # Test
        cnn_feat_att_bilstm_model.eval()

        argmax_labels_list = []
        argmax_predictions_list = []
        debug_pred, debug_sents = [], []
        debug_att_val = []
        ...
        for step, (test_x_comment_tokenized, test_padded_word_tokens_matrix,
            ...
            pred, att_val = cnn_feat_att_bilstm_model(test_padded_word_tokens_matrix, test_padded_char_tokens_matrix)

            debug_pred.append(pred)
            debug_sents.extend(test_x_comment_tokenized)
            debug_att_val.extend(att_val)
            max_predictions, argmax_predictions = pred.max(1)
            argmax_predictions_list.append(argmax_predictions)
            argmax_labels_list.append(test_y_hate_item)

        # debug
        debug_pred = torch.cat(debug_pred, 0)
        debug_pred_np_array = to_np(debug_pred)
        ...
        debug_result(debug_pred_np_array, argmax_predictions_np_array, argmax_labels_np_array, debug_sents,
                     debug_output=f'./log/debug_epoch{epoch + 1}.tsv', heatmap_name=f'./log/plot_epoch{epoch+1}.png')

        if epoch > 8:
            plot_att_val(debug_sents, debug_att_val, argmax_predictions_np_array, argmax_labels_np_array)
