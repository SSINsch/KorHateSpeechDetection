...
import torch
import numpy as np
from utils import prepare_sequence, prepare_char_sequence
...
class NaverReviewDataset(torch.utils.data.Dataset):
    def __init__(self, data_path, tokenizer, tok2idx, char2idx):
        ...
        self.comments = self.data['document']
        self.hate = self.data['label']
...
def collate_basic(data):
    data.sort(key=lambda x: len(x[0]), reverse=True)

    x_comment_tokenized, x_idx_tokens_item, y_hate_item = zip(*data)
    ...
    return x_comment_tokenized, padded_word_tokens_matrix, targets
...
def get_loader(data_path, tokenizer, tok2idx, char2idx, batch_size=256, shuffle=True):
    dataset = NaverReviewDataset(data_path=data_path,
                                 tokenizer=tokenizer,
                                 tok2idx=tok2idx,
                                 char2idx=char2idx)
